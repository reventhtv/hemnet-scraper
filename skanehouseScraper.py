...
for i in range(0,500):
    url = "https://www.hemnet.se/salda/bostader?location_ids%5B%5D=17753&page={}".format(i)
    ...
    df_size = df.size
    # print(df_size)
    df.to_csv('skane_house_prices.csv', encoding='utf-8', index=False, mode='a', header=False)



df_read = pd.read_csv("skane_house_prices.csv", names = ['region', 'price', 'price per sq mtr', 'rooms', 'size','rent'])
print(df_read.head())
...
df_read.to_csv('skane_house_price_clean.csv', encoding='utf-8', index=False)
